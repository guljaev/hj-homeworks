...
// 1. Почему нужно отправлять на сервер fd, а не formData? В реальной практике тоже обычно сначала из объекта FormData делают обычный Object и его уже отправляют? Или могут опправлять на сервер исходный объект FormData?
// 2. Почему при запросе через fetch я получаю не такой же ответ, как через XMLHttpRequest и как с ним работать?
// 3. Я выяснил, что если писать 
    // request.then(res => {
...
// то все заработает. Судя по описание в интернете .json() распарисивает тело (body) возвращаемого ответа. Я попытался описать этот процесс знакомыми операторами:
    ...
    // });
// однако так опять ничего не работает. Объясните пожалуйста, что делает res.json() и как устроен ответ, получаемый при запросе через fetch. На лекции про это ничего не рассказывали.